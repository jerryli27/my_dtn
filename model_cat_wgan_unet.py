        ...
        with tf.variable_scope('content_extractor', reuse=reuse):
            with slim.arg_scope([slim.conv2d], padding='SAME', activation_fn=None, weights_initializer=tf.contrib.layers.xavier_initializer()):
                with slim.arg_scope([slim.batch_norm], decay=0.95, center=True, scale=True, 
                    ...
                    net = slim.conv2d(conv4_1, 512, [4, 4], stride=2, padding='VALID', scope='conv4_2')   # (batch_size, 1, 1, 512)
            ...
            with slim.arg_scope([slim.conv2d_transpose], padding='SAME', activation_fn=None,
                ...
                with slim.arg_scope([slim.batch_norm], decay=0.95, center=True, scale=True, 
                    ...
                    with slim.arg_scope([slim.conv2d], padding='SAME', activation_fn=None,
                        ...
                        concat = tf.concat(3,(net, content_extractor_layers['conv4_1']))
                        net = slim.conv2d_transpose(concat, 256, [3, 3], scope='conv_transpose2_1')  # (batch_size, 8, 8, 256)
                        net = slim.batch_norm(net, scope='bn2')
                        net = slim.conv2d(net, 256, [3, 3], scope='conv_transpose2_2')   # (batch_size, 8, 8, 256)
                        net = slim.batch_norm(net, scope='bn2_2')
                        concat = tf.concat(3,(net, content_extractor_layers['conv3_1']))
                        net = slim.conv2d_transpose(concat, 128, [3, 3], scope='conv_transpose3_1')  # (batch_size, 16, 16, 128)
                        net = slim.batch_norm(net, scope='bn3')
                        net = slim.conv2d(net, 128, [3, 3], scope='conv_transpose3_2')   # (batch_size, 16, 16, 128)
                        net = slim.batch_norm(net, scope='bn3_2')
                        concat = tf.concat(3,(net, content_extractor_layers['conv2_1']))
                        net = slim.conv2d_transpose(concat, 3, [3, 3], activation_fn=tf.nn.tanh, scope='conv_transpose4')   # (batch_size, 32, 32, 3)
                        return net
